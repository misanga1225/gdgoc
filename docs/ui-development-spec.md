...
## 19. Phase2 追加仕様（D-03 レイアウト詳細）
本章は D-03 のレイアウト詳細に関する確定事項のみを定義する。D-03の入力・処理仕様本体は次タスクで確定する。

### 19-1. レイアウト方針
- D-03は `左サイドバー + 右メイン` の2ペインを継続する。
- 右ペイン追加は行わない。
- 右メイン内フォームは固定最大幅（例: `840px`）で扱う。

### 19-2. D-02との関係
- D-03表示中は左サイドバーを表示したままとする。
- D-05表示中のみ左サイドバーを非表示とする。
