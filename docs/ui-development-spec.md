...
- 操作:
  - 単クリック: 患者選択のみ。
  - ダブルクリック: D-05へ遷移（統合前は既存D-05モック画面を開く）。
...
- D-05遷移は左ペインの患者行ダブルクリックを正式導線とする。
- D-05表示中は左サイドバーを表示しない。
- D-05には `一覧画面へ戻る` ボタンを置く（TopHeaderのAppBrand隣）。
- 最新URL解決を使う遷移はバックエンド統合後に実装する。Phase2では遷移枠のみ実装対象とする。

## 19. Phase2 追加仕様（D-03 資料追加・URL発行画面）
本章は D-03 のレイアウト・入力・遷移・アップロード先制御に関する確定事項を定義する。

### 19-1. レイアウト方針
- D-03は単一メイン画面とし、左サイドバーは表示しない。
- 画面上部に `一覧画面へ戻る` を配置し、下部に資料追加フォームを表示する。
- フォーム本体は固定最大幅（例: `840px`）で扱う。

### 19-2. D-02との関係
- D-02からD-03へ遷移した際、D-03表示中は左サイドバーを表示しない。
- D-03のアップロード成功後はD-02へ戻る。
...
### 19-4. 入力項目と編集可否
- `selectedSessionId` 指定あり:
  - 患者名/カルテIDは表示のみ（編集不可）。
- `selectedSessionId` 未指定:
  - 患者名/カルテIDを入力して新規セッションを作成する。

### 19-5. selectedFileId の表示ルール
- `selectedFileId` は参照情報として画面に表示する。
- 表示はファイル名優先とし、ファイル名が取得できない場合は `selectedFileId` を表示する。
- `selectedFileId` は今回のPhase2では差し替え更新ロジックには使用しない。
